...
function LogoUploadSection() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowed.includes(file.type)) {
      toast.error("Formats acceptés : PNG, JPEG, SVG, WebP");
      return;
    }
    ...
    try {
      ...
      setLogoKey(res.data.logo_s3_key);
      toast.success("Logo uploadé — il apparaîtra sur vos prochains exports");
    } catch {
  ...
  return (
    <Section
      ...
      description="Affiché sur les exports PDF, Word et Mémoire technique (max 2 Mo)"
    >
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <label
            htmlFor="logo-upload"
            className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-primary-700
                       bg-primary-50 rounded-xl cursor-pointer hover:bg-primary-100 transition-colors"
          >
            {uploading ? (
          ...
          <p className="text-[10px] text-slate-400">PNG, JPEG, SVG ou WebP — max 2 Mo</p>
        </div>
      </div>
    </Section>
  );
